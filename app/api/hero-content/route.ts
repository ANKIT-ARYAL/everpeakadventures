...
export async function GET() {
  const unauthorized = await requireAdmin("hero-content", "view");
...
export async function PUT(request: Request) {
  const unauthorized = await requireAdmin("hero-content", "edit");
  if (unauthorized) return unauthorized;
  ...
  try {
    ...
    const data = {
      ...
      showPopupVideo: body.showPopupVideo,
      popupVideoUrl: body.popupVideoUrl,
    };
