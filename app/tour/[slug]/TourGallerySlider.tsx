...
export default function TourGallerySlider({ mainImage, galleryImages }: Props) {
  // Put the first gallery image first, followed by the main hero image
  ...
  // Auto-swipe loop every 4 seconds between the two images
  useEffect(() => {
  ...
  if (images.length === 0) {
    return (
      <div className="w-full h-full flex items-center justify-center text-gray-400 text-md bg-gray-50 rounded-xl">
        No images available
      </div>
    );
  }
  ...
  return (
    <div className="relative bg-black rounded-xl overflow-hidden shadow-sm border border-gray-100 h-[320px] group w-full">
      {/* Active single image sliding between gallery image first, then hero */}
      <img 
