...
export default async function TourDetailPage({ params }: PageProps) {
  ...
  const shapedDepartures = tourDepartures
    .map(shapeDeparture)
    .filter(Boolean);
  ...
  const galleryImages = (tour.gallery || []).filter(
  ...
  const elevationProfile: ElevationPoint[] = Array.isArray(tourData.elevationProfile) && tourData.elevationProfile.length > 0
  ...
  return (
    <div className="journey-page min-h-screen bg-[#f4f6f8] font-sans text-gray-800 pb-24 text-justify">
      ...
      <div className="mx-auto px-5 lg:px-20 pt-28 pb-4 text-[13px] font-bold text-gray-400 uppercase tracking-wider">
        ...
        <Link href="/tours" className="hover:text-[#24a0ed] transition-colors">Tours</Link>
        <span className="mx-2">›</span>
            ...
            <div className="journey-panel bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.08)] border border-gray-100 p-6 space-y-6 relative overflow-hidden">
              ...
              <div className="relative z-20 pb-2">
                ...
                <div className="mt-6 text-lg font-medium text-[#112233] flex items-center gap-2">
                  ...
                  Duration: {String(tour.duration).toLowerCase().includes('day') ? tour.duration : `${tour.duration} days`}
                </div>
              </div>
                ...
                <details open className="group border border-[#e6e9ee] rounded-xl overflow-hidden [&_summary::-webkit-details-marker]:hidden mt-4">
                  ...
                  <div className="bg-white">
                    ...
                    <div className="flex flex-col">
                      {[...tour.groupPrices].sort((a: any, b: any) => parseInt(a.groupSize) - parseInt(b.groupSize)).map((gp: any) => (
                        <div key={gp.id} className="flex justify-between items-center px-4 py-3 border-b border-gray-100 last:border-0">
                          ...
                          <div className="w-1/3 text-right">
                            <Link href={`/booking-form/?trip_id=${tour.id}`} className="inline-block bg-[#112233] hover:bg-[#1a2b44] text-white text-[11px] uppercase font-bold px-4 py-2 rounded-md transition-colors">
                              Book
                            </Link>
                          </div>
                        </div>
            ...
            <div className="space-y-4 pb-2">
              ...
              <div className="flex items-center gap-2 mb-4">
                ...
                <div className="text-[16px] text-[#333333]">
                  {tour.rate ? `${tour.rate}.0` : "5.0"}/5 from <a href="#review" className="text-[#00a3cc] hover:underline cursor-pointer">835 reviews</a>
                </div>
              </div>
          ...
          <Stagger className="flex flex-col sm:flex-row sm:flex-wrap gap-4 md:gap-6">
            ...
            {relatedTours.map((item) => {
              ...
              return (
                ...
                >
                  <Link href={`/tour/${item.slug ? item.slug : item.id}`} className="absolute inset-0">
                    {/* Image */}
                    <img 
