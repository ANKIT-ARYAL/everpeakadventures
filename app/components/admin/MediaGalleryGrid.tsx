    ...
    <>
      ...
      <div className="px-5 py-3 border-b border-gray-200 flex flex-col sm:flex-row items-center gap-3">
        ...
        <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
          {(['all', 'image', 'video', 'unused'] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-3 py-1.5 rounded text-lg font-bold capitalize transition-colors ${filter === f ? 'bg-white text-[#112233] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
            >
        ...
        <button
          ...
          disabled={uploading}
          className="inline-flex items-center gap-1.5 bg-[#24a0ed] text-white text-lg font-bold px-4 py-2 rounded-lg hover:bg-[#1289d1] disabled:opacity-60 disabled:cursor-not-allowed shrink-0"
        >
      ...
      {uploadError && (
        <div className="px-5 py-2 bg-rose-50 border-b border-rose-100 text-rose-600 text-md font-bold">
          {uploadError}
        </div>
      )}
      ...
      >
        {uploading ? (
          ...
          <div className="py-8">
            ...
            <p className="text-gray-400 text-lg mt-1">or click to browse your computer</p>
          </div>
        )}
      </div>
...
export function FileLightbox({ file, slots, onClose }: { file: GalleryFile; slots: SlotValue[]; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl px-5 lg:px-20" onClick={(e) => e.stopPropagation()}>
        <div className="p-4 border-b border-gray-200 flex items-center justify-between gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={copy}
              className="inline-flex items-center gap-1.5 text-md font-bold bg-[#112233] text-white px-3 py-1.5 rounded-lg hover:bg-[#1e3a52]"
            >
              {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
              {copied ? 'Copied' : 'Copy URL'}
            </button>
            <button onClick={onClose} className="px-3 py-1.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-lg font-bold text-gray-600">
              Close
            </button>
          </div>
        </div>
        ...
        {actionError && (
          <div className="mx-4 mt-4 px-3 py-2 bg-rose-50 border border-rose-100 text-rose-600 text-md font-bold rounded-lg">
            {actionError}
          </div>
        )}
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-[1fr_220px] gap-4">
          ...
          <div className="space-y-3 text-lg">
            <div>
              ...
              <div className="font-mono text-md text-gray-600 break-all mt-0.5">{file.url}</div>
            </div>
            <div>
            ...
            <div>
              ...
              {file.usedIn.length === 0 ? (
                <p className="text-gray-400 italic text-md mt-0.5">Not used anywhere yet.</p>
              ) : (
                <ul className="mt-1 space-y-1 max-h-32 overflow-y-auto">
                  {file.usedIn.map((u, i) => (
                    <li key={i} className="text-md bg-gray-50 border border-gray-100 rounded px-2 py-1.5 text-gray-600">
        ...
        {file.kind === 'image' && (
          <div className="px-4 pb-4 space-y-4">
            <div className="border border-gray-200 rounded-xl p-4 space-y-3">
              <h4 className="font-bold text-[#112233] uppercase tracking-wide text-md flex items-center gap-1.5">
                ...
                {slots.map((slot) => {
                  ...
                  return (
                    <label key={slot.key} className={`flex items-center gap-2 border rounded-lg px-2.5 py-2 cursor-pointer transition-colors ${active ? 'border-[#24a0ed] bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}>
                      ...
                      <span className="text-md font-bold text-gray-700 leading-tight">{slot.label}</span>
                    </label>
                  );
                })}
              </div>
              <button
                ...
                className={`w-full mt-1 inline-flex items-center justify-center gap-1.5 text-md font-bold rounded-lg px-3 py-2 transition-colors ${savedFlash ? 'bg-emerald-500 text-white' : 'bg-[#24a0ed] text-white hover:bg-[#1289d1]'} disabled:opacity-60`}
              >
            ...
            <div className="border border-gray-200 rounded-xl p-4 space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-gray-200 rounded-lg text-lg outline-none focus:border-[#24a0ed] disabled:bg-gray-50 disabled:text-gray-400"
                />
                <button
                  onClick={saveName}
                  disabled={!file.id || savingName}
                  className="inline-flex items-center gap-1 text-md font-bold bg-[#112233] text-white px-3 py-2 rounded-lg hover:bg-[#1e3a52] disabled:opacity-50"
                >
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-1.5 w-full text-md font-bold bg-rose-50 text-rose-600 border border-rose-100 rounded-lg px-3 py-2 hover:bg-rose-100 disabled:opacity-50"
            >
