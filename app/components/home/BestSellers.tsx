      ...
      <section className="px-5 lg:px-20">
        <Reveal className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
          <div>
            ...
            <div className="font-sans text-lg text-foreground/80">
              <RichText html={subtitle ?? 'Top-rated trekking journeys offering breathtaking views and authentic experiences.'} />
            </div>
          </div>
          <div>
            <Link href="/trekking" className="inline-block border border-foreground/20 hover:border-accent-amber text-foreground hover:bg-accent-amber hover:text-white transition-all duration-300 px-6 py-3 rounded-full text-lg font-semibold">
              View All Treks
            </Link>
          </div>
        </Reveal>
          ...
          {data.map((trek) => (
            ...
            >
              ...
              {(trek.lowestPrice || trek.price) && (
                <div className="absolute top-4 right-4 bg-red-500/95 backdrop-blur-sm text-foreground font-sans font-black text-md px-3 py-1.5 rounded-full shadow-sm pointer-events-none">
