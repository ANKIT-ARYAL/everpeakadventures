...
export default function TourCard({ tour }: { tour: TourCardProps }) {
  const displayPrice = tour.lowestPrice ?? tour.discountedPrice ?? tour.price ?? 0;
  const secondaryMeta = tour.difficulty ?? tour.grade ?? tour.bestTime ?? 'Flexible';
  ...
  return (
    <div className="group relative aspect-[4/5] rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-xl transition-all duration-300">
      <Link href={`/tour/${tour.slug ? tour.slug : tour.id}`} className="absolute inset-0">
        <img
        ...
        <div className="absolute inset-x-0 bottom-0 p-6 flex flex-col justify-end">
          ...
          <div className="h-0 overflow-hidden group-hover:h-auto opacity-0 group-hover:opacity-100 transition-all duration-300">
            <p className="text-white/80 text-[14px] font-sans line-clamp-2 pt-3 border-t border-white/20 mt-2 text-justify">
              {stripHtml(tour.overview || tour.description || '').replace(/^(trip overview|overview|highlights)[\s:]*/i, '').trim() || 'Immerse yourself in a comfortable and unforgettable journey designed for adventure lovers.'}
            </p>
          </div>
        </div>
      </Link>
