        ...
        <div className="site-container px-5 lg:px-20">
            ...
            <div className="mb-10 bg-white rounded-2xl p-6 md:px-8 border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <h2 className="text-xl md:text-2xl font-black text-[#112233] mb-1.5 tracking-tight">
                  Search results for: <span className="text-[#24a0ed]">{searchFilters.join(" • ")}</span>
                </h2>
                <p className="text-gray-500 font-medium text-sm">
                  Found {totalFound} {totalFound === 1 ? 'trek' : 'treks'} matching your criteria.
                </p>
              </div>
              <button 
          ...
          {treks.length === 0 ? (
            <div className="text-center py-24 bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.02)]">
              ...
              <h3 className="text-2xl font-black text-[#112233] mb-3">No treks found</h3>
              <p className="text-gray-500 max-w-md mx-auto mb-8">
                We couldn't find any treks that perfectly match your current filters. Try adjusting your search or clearing some filters.
              </p>
              <button onClick={handleClearFilters} className="bg-[#24a0ed] hover:bg-[#1a85c6] text-white font-bold py-3 px-8 rounded-xl transition-colors cursor-pointer">
                View All Treks
              </button>
            </div>
          ) : (
            <Stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 xl:gap-8">
              {treks.map((trek) => (
                ...
                >
                  <Link href={`/trekking/${trek.slug ? trek.slug : trek.id}`} className="flex flex-col h-full">
                    ...
                    <div className="p-6 flex flex-col flex-1 justify-between">
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-100 text-center mb-6 text-md">
                        <div className="bg-[#f8f9fa] p-3 rounded-xl border border-gray-100 group-hover:bg-[#f0f4f8] transition-colors">
                          ...
                          <span className="font-bold text-[#112233] text-sm">{trek.durationDays}</span>
                        </div>
                        <div className="bg-[#f8f9fa] p-3 rounded-xl border border-gray-100 group-hover:bg-[#f0f4f8] transition-colors">
                          <span className="block text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Difficulty</span>
                          <span className="font-bold text-[#112233] text-sm">{trek.difficulty || 'Moderate'}</span>
                        </div>
                      </div>
