                ...
                >
                  <div>
                    <div className="relative h-48 overflow-hidden bg-gray-100">
                      <img 
                        src={item.heroImage || "https://ml978xhbpkuo.i.optimole.com/cb:t1g8.6c6/w:259/h:68/q:mauto/f:best/https://everpeakadventures.com/wp-content/uploads/2025/03/Untitled-design-123456-e1783511870519.png"}
                        alt={item.title}
                        className={`w-full h-full ${!item.heroImage ? 'object-contain p-4 bg-white' : 'object-cover'} group-hover:scale-105 transition-transform duration-500`}
                      />
                      {item.price && (
                        <div className="absolute top-0 left-0 bg-[#d93838] text-white font-bold text-md px-3 py-1.5 rounded-br-lg shadow-sm oswald">
                          ${(item.discountedPrice ?? item.price).toLocaleString()}
                        </div>
                      )}
                    </div>
                    <div className="p-5">
                      <h3 className="font-bold text-[#222222] text-lg md:text-base line-clamp-2 mb-3 group-hover:text-[#24a0ed] transition-colors">
                        {item.title}
                      </h3>
                    </div>
                  </div>
                  <div className="px-5 pb-5 pt-4 border-t border-gray-100 flex items-center justify-between text-md text-gray-500 font-medium">
