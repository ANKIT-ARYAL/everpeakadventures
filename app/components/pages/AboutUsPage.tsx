  ...
  return (
    <div className="min-h-screen bg-[#f8faf9] font-sans text-[#222222]">
      ...
      <section className="py-20 bg-white">
        ...
        <Stagger className="flex justify-between my-16">
                <StaggerItem className=" p-6 rounded-2xl text-center shadow-md flex flex-col justify-center border border-white/5">
                  <h3 className="text-2xl md:text-3xl font-black  mb-1 oswald">{data.happyTravelers}</h3>
                  <p className="text-xs font-medium tracking-wide">Happy Travelers</p>
                </StaggerItem>
                <StaggerItem className=" p-6 rounded-2xl text-center shadow-md flex flex-col justify-center border border-white/5">
                  <h3 className="text-2xl md:text-3xl font-black mb-1 oswald">{data.yearsExperience}</h3>
                  <p className="text-xs font-medium tracking-wide">Years of Experience</p>
                </StaggerItem>
                <StaggerItem className=" p-6 rounded-2xl text-center shadow-md flex flex-col justify-center border border-white/5">
                  <h3 className="text-2xl md:text-3xl font-black mb-1 oswald">{data.successfulTrips}</h3>
                  <p className="text-xs font-medium tracking-wide">Successful Trips</p>
                </StaggerItem>
                <StaggerItem className=" p-6 rounded-2xl text-center shadow-md flex flex-col justify-center border border-white/5">
                  <h3 className="text-2xl md:text-3xl font-black mb-1 oswald">{data.expertGuides}</h3>
                  <p className="text-xs font-medium tracking-wide">Expert Guides</p>
                </StaggerItem>
              </Stagger>
          </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-[1200px] mx-auto px-5">
          <Stagger className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <StaggerItem className="bg-gradient-to-b from-[#ff4a4a] to-[#e61414] text-white rounded-2xl p-8 shadow-[0_10px_30px_rgba(0,114,255,0.15)] flex flex-col justify-between">
              <div>
                <h3 className="text-xl font-bold uppercase tracking-wider mb-4 oswald">
                  Our Mission
                </h3>
                <RichText
                  html={data.missionText}
                  className=" text-xs md:text-sm leading-relaxed mb-8"
                />
              </div>
