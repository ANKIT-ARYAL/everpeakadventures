...
export default function RouteMap({ itinerary, chartTitle }: Props) {
  ...
  const points = days.map((d, i) => {
    const x =
      PAD.left + (i / (days.length - 1 || 1)) * (W - PAD.left - PAD.right);
    const y =
      PAD.top +
      (1 - ((d.elev ?? 0) - minE) / span) * (H - PAD.top - PAD.bottom);
    return {
      x,
      y,
      day: d.day,
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div>
          ...
          <h3 className="text-lg font-bold text-[#112233]">{chartTitle}</h3>
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 bg-[#24a0ed] hover:bg-[#112233] text-white font-bold px-5 py-2.5 rounded-lg text-md uppercase tracking-wider transition-colors"
          >
            {playing ? (
          ...
          <button
            ...
            className="inline-flex items-center gap-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold px-4 py-2.5 rounded-lg text-md uppercase tracking-wider transition-colors"
          >
      ...
      <div className="mt-4 border border-gray-100 bg-gray-50 rounded-xl p-4 min-h-[96px] flex flex-col justify-center">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[11px] text-gray-400 font-bold">
            {marker.elev.toLocaleString()} m
          </span>
        </div>
        <h4 className="font-bold text-lg text-[#112233]">{marker.title}</h4>
        {marker.desc && (
          <RichText
            html={marker.desc}
            className="text-md text-gray-600 mt-1 leading-relaxed"
          />
        )}
      </div>
    </div>
