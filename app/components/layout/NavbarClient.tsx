...
import { 
  ...
  Navigation, Tent, Footprints, Users, Sun, 
  MessageCircle, Mail, MapPin, ChevronRight,
  ChevronUp
} from 'lucide-react';
  ...
  return (
    <>
      <header className={`fixed top-0 w-full z-50 ${headerClasses}`}>
        {/* DESKTOP & TABLET NAV (Supports down to 1024px cleanly) */}
        <div className="hidden lg:flex h-20 items-center justify-between px-4 xl:px-20">
          <Link href="/" className="flex items-center shrink-0">
            <img src={logoImage || "/logo.png"} alt="Ever Peak Adventures" className="h-11 xl:h-14 w-auto object-contain drop-shadow-sm" />
          </Link>

          <div className="hidden lg:flex xl:gap-5 items-center gap-2.5 font-bold text-[11px] xl:text-[13px] text-white tracking-wider">
            <Link href="/" className="hover:text-accent-amber transition-colors uppercase whitespace-nowrap">Home</Link>
            ...
            >
              <div className="flex items-center gap-0.5 hover:text-accent-amber transition-colors cursor-pointer h-full whitespace-nowrap">
                <Link href="/tour-destination/nepal" className="uppercase">Nepal</Link>
                <ChevronDown className="w-3.5 h-3.5" />
              </div>

              {activeDropdown === 'nepal' && (
                <div className="absolute top-full pt-2 left-1/2 -translate-x-1/2 w-[1050px] bg-white rounded-b-2xl shadow-2xl border-t border-gray-100 flex overflow-hidden text-[#112233] z-50">
                  <div className="w-[280px] bg-gray-50 flex flex-col py-4 border-r border-gray-100 shrink-0">
                    {nepalTabs.map((tab: any, idx: number) => {
                      ...
                      return (
                        ...
                        >
                          <span className="text-[13px] normal-case tracking-normal">{tab.name}</span>
                        </div>
                      )
                    })}
                  </div>

                  <div className="flex-1 p-6 xl:p-8 bg-white flex flex-col">
                            ...
                            {nepalTabs[activeNepalTabIdx].items.map((item: any) => (
                              <Link key={item.slug} href={item.type === 'trek' ? `/trekking/${item.slug}` : `/tour/${item.slug}`} onClick={closeDesktopMenu} className="flex items-center justify-between group p-3 rounded-xl border border-gray-100 hover:border-[#112233]/20 hover:shadow-sm transition-all bg-white">
                                <div className="flex items-center gap-3 overflow-hidden">
                                  ...
                                  <span className="text-[13px] font-bold text-[#112233] truncate normal-case tracking-normal group-hover:text-accent-amber transition-colors">{item.title} - {item.durationDays}</span>
                                </div>
            ...
            >
              ...
              {activeDropdown === 'trekking' && (
                <div className="absolute top-full pt-2 left-1/2 -translate-x-1/2 w-[1050px] bg-white rounded-b-2xl shadow-2xl border-t border-gray-100 flex overflow-hidden z-50">
                  <div className="w-72 bg-gray-50 flex flex-col py-4 border-r border-gray-100 shrink-0">
                    {nepalTrekkingTabs.map((region: any, idx: number) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[13px] normal-case tracking-normal">{region.name}</span>
                        </div>
                      )
                    })}
                  </div>
                        ...
                        <div className="grid grid-cols-2 gap-x-6 gap-y-3 mb-8">
                          {nepalTrekkingTabs[activeTrekkingTabIdx].treks.map((trek: any) => (
                            <Link key={trek.slug} href={`/trekking/${trek.slug}`} onClick={closeDesktopMenu} className="flex items-center justify-between group p-3 rounded-xl border border-gray-100 hover:border-[#112233]/20 hover:shadow-sm transition-all bg-white">
                              <div className="flex items-center gap-3 overflow-hidden">
                                <Footprints className="w-4 h-4 text-[#112233]/40 group-hover:text-accent-amber shrink-0" />
                                <span className="text-[13px] font-bold text-[#112233] truncate normal-case tracking-normal group-hover:text-accent-amber transition-colors">{trek.title} - {trek.durationDays}</span>
                              </div>
                              <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-accent-amber shrink-0" />
                            </Link>
                          ))}
            ...
            >
              ...
              {activeDropdown === 'tours' && (
                <div className="absolute top-full pt-2 left-1/2 -translate-x-1/2 w-[1050px] bg-white rounded-b-2xl shadow-2xl border-t border-gray-100 flex overflow-hidden text-[#112233] z-50">
                  ...
                  <div className="flex-1 p-6 xl:p-8 bg-white flex flex-col min-h-[400px]">
                          ...
                          {nepalToursTabs[activeToursTabIdx].tours?.length > 0 ? (
                            nepalToursTabs[activeToursTabIdx].tours.map((tour: any) => (
                              <Link key={tour.slug} href={`/tour/${tour.slug}`} onClick={closeDesktopMenu} className="flex items-center justify-between group p-3 rounded-xl border border-gray-100 hover:border-[#112233]/20 hover:shadow-sm transition-all bg-white">
                                <div className="flex items-center gap-3 overflow-hidden">
                                  <Footprints className="w-4 h-4 text-[#112233]/40 group-hover:text-accent-amber shrink-0" />
                                  <span className="text-[13px] font-bold text-[#112233] truncate normal-case tracking-normal group-hover:text-accent-amber transition-colors">{tour.title} - {tour.durationDays}</span>
                                </div>
                                <ChevronRight className="w-4 h-4 text-gray-300 group-hover:text-accent-amber shrink-0" />
                              </Link>
                            ))
            ...
            >
              ...
              {activeDropdown === 'about' && (
                <div className="absolute top-[80px] left-0 w-64 bg-white rounded-xl shadow-xl border border-gray-100 py-3 animate-in fade-in slide-in-from-top-4 duration-200 z-50">
            ...
            <Link href="/blog" className="hover:text-accent-amber transition-colors uppercase whitespace-nowrap">Blog</Link>
            <Link href="/contact-us" className="hover:text-accent-amber transition-colors uppercase whitespace-nowrap">Contact</Link>
            
            <button 
              aria-label="Search" 
              onClick={() => setSearchModalOpen(true)}
              className="ml-1 p-1.5 rounded-full hover:bg-black/5 transition-colors cursor-pointer text-white shrink-0"
            >
              <Search className="w-4 h-4 xl:w-5 xl:h-5 hover:text-accent-amber transition-colors" />
            </button>
          </div>
        </div>
