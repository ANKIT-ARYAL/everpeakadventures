      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-black text-[#112233] oswald uppercase tracking-wide">Why Choose Us Items</h1>
            <span className="bg-gray-100 text-gray-600 font-bold px-2.5 py-0.5 rounded-full">
              {items.length} items
            </span>
          </div>
          <p className="text-gray-500 mt-1">Manage the icon list items displayed in the Why Choose Us section.</p>
        </div>
                ...
                items.map((item, index) => (
                  <tr key={item.id} className="hover:bg-[#fcfcfc] transition-colors group">
                    ...
                    <td className="py-3 px-4 font-bold text-[#112233]">
                      <Link href={`/admin/why-choose-us/items/${item.id}/edit`} className="hover:text-[#24a0ed]">
                        {item.title}
                      </Link>
                    </td>
