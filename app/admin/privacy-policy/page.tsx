...
export default function PrivacyPolicyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to save Privacy Policy page');
    ...
    <form onSubmit={handleSubmit} className="max-w-5xl mx-auto space-y-6 text-xs text-gray-800 font-sans pb-20">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-black uppercase text-[#112233]">Privacy Policy</h1>
        </div>
        <div className="flex items-center gap-2">
          <ToggleShow model="privacy-policy" resource="privacy-policy" id="__single__" published={form.published ?? true} />
          <button 
            type="submit" 
            disabled={saving}
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div>
          ...
          <input type="text" name="title" value={form.title} onChange={handleChange} className="w-full p-3 border rounded-lg text-sm font-medium focus:border-[#24a0ed] outline-none" placeholder="Privacy Policy" />
        </div>
