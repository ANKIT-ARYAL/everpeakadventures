      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-black text-[#112233] oswald uppercase tracking-wide">Activities</h1>
        ...
        <AddNewButton href="/admin/activities/new" label="Add New Hero" />
      </div>
            ...
            <tbody className="divide-y divide-gray-100">
              {activities.length === 0 ? (
                ...
                activities.map((activity: any) => (
                  <tr key={activity.id} className="hover:bg-[#fcfcfc] transition-colors group">
                    <td className="py-3 px-4 font-bold text-[#112233]">
                      <Link href={`/admin/activities/${activity.id}/edit`} className="hover:text-[#24a0ed]">
                        {activity.slug}
                      </Link>
        ...
        {activities.length === 0 ? (
          ...
          activities.map((activity: any) => (
            <div key={activity.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-4">
              <Link href={`/admin/activities/${activity.id}/edit`} className="font-bold text-[#112233] hover:text-[#24a0ed] block truncate">
                {activity.slug}
              </Link>
              <span className="block text-[10px] text-gray-400 font-normal truncate">ID: {activity.id}</span>
              <div className="mt-3 text-[11px]">
                ...
                <span className="text-gray-500 font-medium line-clamp-2">{activity.description || '—'}</span>
              </div>
