...
export default async function AdminDashboardPage() {
  ...
  return (
    <AdminPageLayout
      title="Dashboard"
      description="Your website overview, content shortcuts, database metrics, and system access status."
      actions={
        <div className="bg-emerald-50 text-emerald-700 font-bold px-3 py-1.5 rounded-lg border border-emerald-200 flex items-center gap-1.5 text-sm whitespace-nowrap">
        ...
        {overviewCards.length > 0 && (
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-200 overflow-hidden">
            <div className="mb-4 sm:mb-6">
              <h2 className="font-bold text-gray-900 uppercase tracking-wider text-sm sm:text-base">
                Website Content Overview
              </h2>
              <p className="text-gray-500 mt-0.5 text-sm">Quick access to all your website content. Click any card to manage.</p>
            </div>
              ...
              {overviewCards.map((card, idx) => {
                ...
                return (
                  <Link 
                    key={idx}
                    href={card.href}
                    className="p-4 sm:p-5 rounded-xl border border-gray-200 bg-white hover:bg-gray-50 hover:shadow-sm transition-all flex flex-col justify-between group min-w-0 overflow-hidden"
                  >
                    <div className="flex items-start justify-between mb-4 gap-2">
                      <div className="min-w-0 flex-1">
                        ...
                        <span className="text-2xl sm:text-3xl font-black text-gray-900 block truncate">
                          {card.count}
                        </span>
                      </div>
                      <div className={`p-2.5 rounded-xl bg-gray-50 border border-gray-100 group-hover:scale-110 transition-transform ${card.color} shrink-0`}>
                        <Icon className="w-5 h-5" />
                      </div>
                    </div>

                    <div className="pt-3 border-t border-gray-100 flex items-center justify-between text-[11px]">
                      <span className="text-gray-500 font-medium group-hover:text-blue-600 transition-colors truncate block w-full">
                        {card.desc} →
                      </span>
                    </div>
                  </Link>
        ...
        {can("bookings") && (
          <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-200 overflow-hidden">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-4">
              <h2 className="font-bold text-gray-900 uppercase tracking-wider flex items-center gap-2 text-sm sm:text-base">
                <Briefcase className="w-4 h-4 text-orange-500 shrink-0" /> Recent Booking Requests
              </h2>
              <Link href="/admin/bookings" className="text-sm font-medium text-blue-600 hover:underline whitespace-nowrap">
                View all →
              </Link>
            </div>
            {recentBookings.length === 0 ? (
              <p className="py-6 text-center text-gray-400 font-medium text-sm">No booking requests yet.</p>
            ) : (
                ...
                {recentBookings.map((b) => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <span className="block font-bold text-gray-900 truncate">{b.fullName} — {b.tripTitle}</span>
                    ...
                    <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start shrink-0 pl-5 sm:pl-0">
                      <span className="block font-bold text-gray-900 text-sm truncate">{b.estimatedTotal}</span>
                      <span className="block text-[11px] text-gray-500 sm:mt-0.5">
