...
export default async function AdminTrustItemsPage() {
  ...
  const tableRows = items.map((item, index) => [
    ...
    <div key="title" className="min-w-0">
      <Link href={`/admin/trust-items/${item.id}/edit`} className="font-bold text-[#112233] hover:text-[#24a0ed] block break-words">
        {item.title}
      </Link>
    </div>,
    <span key="subtitle" className="text-gray-600 font-medium block max-w-md line-clamp-2 break-words">
      {item.subtitle}
    </span>,
  ...
  return (
    <AdminPageLayout
      title="Trust Items"
      description="Manage trust badges, certifications, and partner highlights."
      actions={
    ...
    >
      <div className="flex flex-col gap-6 pb-10 min-w-0">
        ...
        <ResponsiveTable
          headers={['#', 'Title', 'Subtitle', 'Icon', 'Order', 'Actions']}
          rows={tableRows}
          data={items}
          emptyText="No trust items found."
          ...
          mobileCards={(_row, item, index) => (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-5 min-w-0 overflow-hidden">
              <div className="flex items-start justify-between gap-3">
                <Link href={`/admin/trust-items/${item.id}/edit`} className="font-bold text-[#112233] hover:text-[#24a0ed] min-w-0 flex-1 block text-lg sm:text-xl leading-tight break-words">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3 mt-4 text-xs">
                <div className="min-w-0 sm:col-span-2">
                  ...
                  <span className="text-gray-600 font-medium line-clamp-3 break-words">{item.subtitle}</span>
                </div>
