...
export default function DepartureForm({ initialData, isEditing = false, treks = [], tours = [] }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-4xl xl:max-w-none mx-auto space-y-6 text-md text-gray-800">
          ...
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1 w-fit">
            {['trek', 'tour'].map((t) => (
              <button
                ...
                className={`px-6 py-2 rounded text-md font-bold uppercase transition-colors ${tripType === t ? 'bg-white text-[#112233] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                {t === 'trek' ? 'Trek' : 'Tour'}
              </button>
            ))}
