  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-md text-gray-500 mt-1 leading-relaxed">{section.description}</p>
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-[#112233] text-lg group-hover:text-[#24a0ed] transition-colors truncate">{link.label}</div>
        {note ? (
          <p className="text-md text-gray-400 mt-0.5 truncate">{note}</p>
        ) : (
  ...
  return (
    <AdminPageLayout
      ...
      actions={
        <Link 
          href="/admin/pages" 
          className="text-sm font-bold text-gray-500 hover:text-gray-900 bg-white border border-gray-200 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-colors"
        >
          ← All Pages
        </Link>
      }
    >
      <div className="max-w-[1200px] xl:max-w-none text-lg">
        {grouped ? (
          <div className="space-y-6">
            {grouped.map((g) => {
              ...
              return (
                <div key={g.label}>
                  <h2 className="text-sm font-black uppercase tracking-widest text-gray-500 mb-3 px-1">{g.label}</h2>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            <p className="text-lg text-gray-400 italic">No pages available in this section.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {links.map((link) => (
