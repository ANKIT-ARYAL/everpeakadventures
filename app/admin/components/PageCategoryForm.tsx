...
export default function PageCategoryForm({ parents = [], initialData, isEditing = false }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing ? `/api/page-categories/${initialData.id}` : '/api/page-categories';
      ...
      setTimeout(() => router.push('/admin/pages/categories'), 400);
    } catch (err: any) {
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-[#112233] uppercase tracking-wide">
            {isEditing ? 'Edit Page Category' : 'Add New Page Category'}
          </h1>
          <p className="text-gray-500 mt-1">
            {form.parentId ? 'This will be a sub-category under the selected main category.' : 'Main categories can contain sub-categories and pages.'}
          </p>
        </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" name="name" value={form.name} onChange={handleChange} placeholder="e.g. About Company" className={inputCls} />
          </div>
          <div>
            <label className={labelCls}>Parent (Main Category)</label>
