...
export default function SiteSettingsPage() {
  ...
  const inputCls = 'w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none';
  const monoCls = `${inputCls} font-mono text-md`;
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-5xl xl:max-w-none mx-auto space-y-6 text-md text-gray-800 font-sans pb-20">
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div>
          <label className="block font-bold mb-1">Login Page Hero Image</label>
          <MediaUploader value={form.loginHeroImage} onChange={(url) => setForm(prev => ({ ...prev, loginHeroImage: url }))} label="Upload Login Image" heightClass="h-32" />
        </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div>
          ...
          <input type="text" name="emergencyLabel" value={form.emergencyLabel} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. Emergency SOS (24/7):" />
        </div>

        <div>
          <label className="block font-bold mb-1">Landline</label>
          <input type="text" name="emergencyLandline" value={form.emergencyLandline} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. +977 98000000" />
        </div>

        <div>
          <label className="block font-bold mb-1">Phone</label>
          <input type="text" name="emergencyPhone" value={form.emergencyPhone} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. 9851093960" />
        </div>

        <div>
          <label className="block font-bold mb-1">WhatsApp</label>
          <input type="text" name="whatsapp" value={form.whatsapp} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. 9851093960" />
        </div>
      </div>

      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        <h2 className="font-bold text-gray-800 uppercase tracking-wider border-b pb-2">SMTP Configuration (Email Server)</h2>
        <p className="text-gray-500 text-md mb-4">Required for sending password reset emails and contact form submissions. Example for Gmail: smtp.gmail.com, 587, your email, your app password.</p>

        <div>
          <label className="block font-bold mb-1">SMTP Host</label>
          <input type="text" name="smtpHost" value={form.smtpHost} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. smtp.gmail.com" />
        </div>

        <div>
          <label className="block font-bold mb-1">SMTP Port</label>
          <input type="text" name="smtpPort" value={form.smtpPort} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. 587" />
        </div>

        <div>
          <label className="block font-bold mb-1">SMTP Username</label>
          <input type="text" name="smtpUser" value={form.smtpUser} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. you@gmail.com" />
        </div>

        <div>
          <label className="block font-bold mb-1">SMTP App Password</label>
          <input type="password" name="smtpPassword" value={form.smtpPassword} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="Enter app password" />
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        ...
        <div>
          ...
          <input type="text" name="email" value={form.email} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. info@example.com" />
        </div>

        <div>
          <label className="block font-bold mb-1">Address Line 1</label>
          <input type="text" name="addressLine1" value={form.addressLine1} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. Payutar Dhara" />
        </div>

        <div>
          <label className="block font-bold mb-1">Address Line 2</label>
          <input type="text" name="addressLine2" value={form.addressLine2} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. Kathmandu, Nepal" />
        </div>

        <div>
          <label className="block font-bold mb-1">Address Map URL</label>
          <input type="text" name="addressMapUrl" value={form.addressMapUrl} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. https://maps.app.goo.gl/..." />
        </div>
      </div>

      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        <h2 className="font-bold text-gray-800 uppercase tracking-wider border-b pb-2">Social Media URLs</h2>

        <div>
          <label className="block font-bold mb-1">Facebook URL</label>
          <input type="text" name="facebookUrl" value={form.facebookUrl} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. https://facebook.com/everpeak" />
        </div>

        <div>
          <label className="block font-bold mb-1">Instagram URL</label>
          <input type="text" name="instagramUrl" value={form.instagramUrl} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. https://instagram.com/everpeak" />
        </div>

        <div>
          <label className="block font-bold mb-1">YouTube URL</label>
          <input type="text" name="youtubeUrl" value={form.youtubeUrl} onChange={handleChange} className="w-full p-3 border rounded-lg text-lg font-medium focus:border-[#24a0ed] outline-none" placeholder="e.g. https://youtube.com/c/everpeak" />
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
        <div className="flex items-center justify-between border-b pb-2">
          ...
          <button type="button" onClick={addColumn} className="bg-[#24a0ed] hover:bg-[#1a85c6] text-white font-bold px-3 py-1.5 rounded-lg text-md">+ Add Column</button>
        </div>
        <p className="text-gray-500">Each column shows a title plus its list of links. Leave a column empty to delete it.</p>

        {form.footerColumns.length === 0 && <p className="text-gray-400 text-lg italic">No columns yet — add one to start editing.</p>}
        ...
        {form.footerColumns.map((col, ci) => (
          <div key={ci} className="border border-gray-200 rounded-lg p-4 space-y-3">
            ...
            <button type="button" onClick={() => addColumnLink(ci)} className="text-[#24a0ed] hover:text-[#1a85c6] font-bold text-md">
              + Add Link
            </button>
          </div>
        ))}
      ...
      {(['associations', 'payments'] as const).map((group) => (
        <div key={group} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
          <div className="flex items-center justify-between border-b pb-2">
            ...
            <button type="button" onClick={() => addLogo(group)} className="bg-[#24a0ed] hover:bg-[#1a85c6] text-white font-bold px-3 py-1.5 rounded-lg text-md">+ Add Logo</button>
          </div>
          ...
          {form.footerLogos[group].length === 0 && <p className="text-gray-400 text-lg italic">No logos yet — add one to start editing.</p>}
