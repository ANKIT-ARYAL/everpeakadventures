...
export default function TermsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch('/api/terms-page');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to save Terms page');

      toast.success('Terms page saved successfully!');
      router.push('/admin/terms-page');
      router.refresh();
    } catch (err: any) {
      toast.error(err.message);
    } finally {
    ...
    <form onSubmit={handleSubmit} className="max-w-5xl mx-auto space-y-6 text-xs text-gray-800 font-sans pb-20">
      ...
      <SectionCard title="Page Header" defaultOpen>
        <div>
          ...
          <input type="text" name="title" value={form.title} onChange={handleChange} className="w-full p-3 border rounded-lg text-sm font-medium focus:border-[#24a0ed] outline-none" placeholder="Terms and Conditions" />
        </div>
