...
```ts
export const EMPTY_ROUTE_MAP: RouteMapData = {
  peaks: [],
  routePoints: [],
  routeSegments: [],
};
...
const POINT_TYPES: RoutePointType[] = [
  "start",
  "end",
  "trek",
  "acclimatization",
...
const inputCls = "w-full px-2 py-1 border border-gray-200 rounded text-md";
...
export default function RouteMapEditor({
  ...
  const setPeak = (i: number, patch: Partial<PeakData>) =>
    set({
      peaks: d.peaks.map((p, idx) => (idx === i ? { ...p, ...patch } : p)),
    });
  const setPoint = (i: number, patch: Partial<RoutePointData>) =>
    set({
      routePoints: d.routePoints.map((p, idx) =>
        idx === i ? { ...p, ...patch } : p,
      ),
    });
  const setSegment = (i: number, patch: Partial<RouteSegmentData>) =>
    set({
      routeSegments: d.routeSegments.map((p, idx) =>
        idx === i ? { ...p, ...patch } : p,
      ),
    });
  ...
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          <label className="block font-bold text-gray-600 mb-1 text-[10px] uppercase tracking-wider">
            Title override
          </label>
          <input
            className={inputCls}
            value={d.title || ""}
            placeholder={d.title || "Defaults to package title"}
        ...
        <div>
          ...
          <input
            className={inputCls}
            value={d.subtitle || ""}
            placeholder="e.g. Detailed route map with elevation profile"
        ...
        <div>
          ...
          <input
            className={inputCls}
            value={d.brandTagline || ""}
            placeholder="Walk, Explore and Discover"
        ...
        <div>
          ...
          <input
            className={inputCls}
            value={d.footerUrl || ""}
            placeholder="www.nepalhikingteam.com"
      ...
      <div className="border-t border-gray-100 pt-3">
        <div className="flex items-center justify-between mb-2">
          <h4 className="font-bold text-gray-700 uppercase tracking-wider text-md">
            Peaks
          </h4>
          <button
            type="button"
            onClick={() =>
              set({
                peaks: [...d.peaks, { name: "", elevation: 0, x: 0, y: 0 }],
              })
            }
            className="bg-[#112233] text-white px-3 py-1.5 rounded text-[10px] font-bold flex items-center gap-1"
          >
            <Plus className="w-3.5 h-3.5" /> Add Peak
          </button>
        </div>
        {d.peaks.length === 0 && (
          <p className="text-[11px] text-gray-400 italic">No peaks defined.</p>
        )}
          ...
          {d.peaks.map((p, i) => (
            <div key={i} className="flex items-center gap-2 flex-wrap">
              <input
                ...
                placeholder="Name"
                value={p.name}
                onChange={(e) => setPeak(i, { name: e.target.value })}
              />
              <input
                type="number"
                ...
                value={p.elevation || ""}
                onChange={(e) =>
                  setPeak(i, { elevation: Number(e.target.value) })
                }
              />
              <input
                type="number"
                ...
                placeholder="X"
                value={p.x || ""}
                onChange={(e) => setPeak(i, { x: Number(e.target.value) })}
              />
              <input
                type="number"
                className={inputCls + " w-16"}
                placeholder="Y"
                value={p.y || ""}
                onChange={(e) => setPeak(i, { y: Number(e.target.value) })}
              />
              <button
                type="button"
                onClick={() =>
                  set({ peaks: d.peaks.filter((_, idx) => idx !== i) })
                }
      ...
      <div className="border-t border-gray-100 pt-3">
          ...
          <button
            ...
            onClick={() =>
              set({
                routePoints: [
                  ...d.routePoints,
                  { id: "", name: "", elevation: 0, x: 0, y: 0, type: "trek" },
                ],
              })
            }
            className="bg-[#112233] text-white px-3 py-1.5 rounded text-[10px] font-bold flex items-center gap-1"
        ...
        {d.routePoints.length === 0 && (
          <p className="text-[11px] text-gray-400 italic">
            No route points defined.
          </p>
        )}
        <div className="space-y-2">
          {d.routePoints.map((p, i) => (
            <div key={i} className="flex items-center gap-2 flex-wrap">
              <input
                ...
                placeholder="id"
                value={p.id}
                onChange={(e) => setPoint(i, { id: e.target.value })}
              />
              <input
              ...
              <input
                ...
                value={p.elevation || ""}
                onChange={(e) =>
                  setPoint(i, { elevation: Number(e.target.value) })
                }
              />
              <input
                type="number"
              ...
              <select
                ...
                value={p.type}
                onChange={(e) =>
                  setPoint(i, { type: e.target.value as RoutePointType })
                }
              >
                {POINT_TYPES.map((t) => (
              ...
              <button
                ...
                onClick={() =>
                  set({
                    routePoints: d.routePoints.filter((_, idx) => idx !== i),
                  })
                }
                className="text-red-500 hover:bg-red-50 p-1.5 rounded"
              >
      ...
      <div className="border-t border-gray-100 pt-3">
        ...
        {d.routeSegments.length === 0 && (
          <p className="text-[11px] text-gray-400 italic">
            No route segments defined.
          </p>
        )}
        <div className="space-y-2">
          {d.routeSegments.map((p, i) => (
            <div key={i} className="flex items-center gap-2 flex-wrap">
              <input
                ...
                placeholder="From (point id)"
                value={p.from}
                onChange={(e) => setSegment(i, { from: e.target.value })}
              />
              <span className="text-gray-400">→</span>
              <input
                className={inputCls + " w-40"}
                placeholder="To (point id)"
                value={p.to}
                onChange={(e) => setSegment(i, { to: e.target.value })}
              />
              <select
                className={inputCls + " w-32"}
                value={p.type}
                onChange={(e) =>
                  setSegment(i, { type: e.target.value as SegmentType })
                }
              >
                {SEGMENT_TYPES.map((t) => (
                  <option key={t} value={t}>
              ...
              <button
                ...
                onClick={() =>
                  set({
                    routeSegments: d.routeSegments.filter(
                      (_, idx) => idx !== i,
                    ),
                  })
                }
...
import RouteMapEditor, {
  EMPTY_ROUTE_MAP,
} from "@/app/components/admin/RouteMapEditor";
```
