      ...
      <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-5">
        ...
        >
          ...
          <span className="min-w-0">
            <span className="block font-bold text-[#112233] uppercase tracking-wide text-lg truncate">
              {title}
            </span>
            {subtitle && (
...
export default function FieldGrid({
  ...
  return (
    <div
      className={`grid grid-cols-1 sm:grid-cols-2 ${colsCls} gap-3 ${className}`}
    >
      {children}
    </div>
  );
...
Replace the `{ href: '/admin/bookings', ... }` entry in `navItems` so bookings is _not_ a plain item. Inside `renderNav()`, right after the `filteredNav.map(...)`, keep bookkeeping minimal:
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-1">
        {sections.map((s) => (
          <a
            ...
            className="shrink-0 px-3 py-1.5 rounded-full bg-white border border-gray-200 text-md font-bold text-gray-600 hover:border-[#24a0ed] hover:text-[#24a0ed]"
          >
            {s}
          </a>
        ))}
...
Run: `npx tsc --noEmit` and `npx eslint app/components/admin/StickySectionNav.tsx app/admin/treks/TrekForm.tsx app/admin/tours/TourForm.tsx` (only pre-existing `any`/unused errors tolerated — none _new_).
